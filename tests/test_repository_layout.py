...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "de"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_germany_namespace_present() -> None:
    """Germany federal scope: the only jurisdiction directory is de/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"de"}, f"unexpected jurisdiction dirs: {names - {'de'}}"


def test_de_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "de" / marker).is_dir(), f"missing de/{marker}"
